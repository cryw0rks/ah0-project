<template>
  <div class="user">
    <h1>kami edit <router-link to="/kami/manage">[back to manage]</router-link></h1>
    <table class="fit-form">
      ...
      <tr>
        ...
        <td>:</td>
        <td>
        	<textarea v-model="descriptionzz" id="descriptionzz" placeholder="this is story about young boy who stupid and ....."></textarea>
        </td>
      </tr>
      <tr>
...
export default {
  ...
  async mounted() {
    ...
    if (dataContent["status"] == "profile") {
      dataContent["status"] = "only on profile"
    }
    this.status = dataContent["status"]
  }
};
</script>
